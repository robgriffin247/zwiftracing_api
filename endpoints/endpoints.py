...
def get_club(id: int, from_id: int = None):
    response = httpx.get(url=f"{ZRAPP_API_URL}clubs/{id}/{from_id if from_id else ''}",
                         headers=HEADER)
...
def get_results(id: int):
    response = httpx.get(url=f"{ZRAPP_API_URL}results/{id}",
                         headers=HEADER)
    response.raise_for_status()
    return response.json()
...
def get_zp_results(id: int):
    response = httpx.get(url=f"{ZRAPP_API_URL}zp/{id}/results",
                         headers=HEADER)
    response.raise_for_status()
    return response.json()
...
def get_rider(id: int, epoch: int = None):
    response = httpx.get(url=f"{ZRAPP_API_URL}riders/{id}/{epoch if epoch else ''}",
                         headers=HEADER)
    response.raise_for_status()
    return response.json()
...
def get_riders(ids: list[int], epoch: int = None):
    response = httpx.post(url=f"{ZRAPP_API_URL}riders/{epoch if epoch else ''}",
                          headers=HEADER,
                          json=ids)
    response.raise_for_status()
    return response.json()
...
if __name__=="__main__":
    ...
    club_from = get_club(20650, 4598636)
    with open("endpoints/data/club_from.json", "w") as f:
         json.dump(club_from, f)
